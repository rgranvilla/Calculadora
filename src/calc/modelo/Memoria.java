...
public class Memoria {
	...
	private enum TipoComando {
			ZERAR, NUMERO, DIV, MULT, SUB, SOMA, IGUAL, VIRGULA, ALTERNAR_SINAL, PORCENTAGEM;
	};
	...
	public void processarComando(String texto) {
		...
		if(tipoComando == null) {
			...
			substituir = false;
			ultimaOperacao = null;
		} else if(tipoComando == TipoComando.ALTERNAR_SINAL && textoAtual.contains("-")) {
	...
	private String obterResultadoOperacao() {
		...
		if(ultimaOperacao == TipoComando.SOMA) {
			...
			resultado = numeroBuffer * (0.01) * numeroAtual;
		}
	...
	private TipoComando detectarTipoComando(String texto) {
		...
		try {
			...
			if("AC".equals(texto)) {
				...
				return TipoComando.ALTERNAR_SINAL;
			} else if("%".equals(texto)) {
